...
import type { IPlaylistDetail, ISong, PlaylistsType } from "../../types";
...
export default function SearchList(props: ISearchListProps) {
    ...
    const changeSongHandler = React.useCallback(
        (song: ISong) => {
        ...
        <div>
            ...
            <div className="grid grid-cols-5 gap-5">
                {props.playlists.map((playlist) => {
                    return <VerticalPlaylist playlist={playlist} key={playlist.id} />;
                })}
            </div>
        </div>
    );
